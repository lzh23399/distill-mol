...
def safe_div(num, den, eps=1e-08):
...
class EGNN(nn.Module):
    ...
    def __init__(self, dim, edge_dim=0, m_dim=16, fourier_features=0, num_nearest_neighbors=0, dropout=0.0, init_eps=0.001, norm_feats=False, norm_coors=False, norm_coors_scale_init=0.01, update_feats=True, update_coors=True, only_sparse_neighbors=False, valid_radius=float('inf'), m_pool_method='sum', soft_edges=False, coor_weights_clamp_value=None):
        super().__init__()
        ...
        self.coors_mlp = nn.Sequential(nn.Linear(m_dim, m_dim * 4), dropout, SiLU(), nn.Linear(m_dim * 4, 1)) if update_coors else None
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None):
        ...
        if use_nearest:
            ...
            if exists(mask):
                ...
                ranking.masked_fill_(~rank_mask, 100000.0)
            if exists(adj_mat):
...
class EGNN_Network(nn.Module):
    ...
    def __init__(self, *, depth, dim, num_tokens=None, num_edge_tokens=None, num_positions=None, edge_dim=0, num_adj_degrees=None, adj_dim=0, global_linear_attn_every=0, global_linear_attn_heads=8, global_linear_attn_dim_head=64, num_global_tokens=4, **kwargs):
        super().__init__()
        ...
        for ind in range(depth):
            ...
            self.layers.append(nn.ModuleList([GlobalLinearAttention(dim=dim, heads=global_linear_attn_heads, dim_head=global_linear_attn_dim_head) if is_global_layer else None, EGNN(dim=dim, edge_dim=edge_dim + adj_dim, norm_feats=True, **kwargs)]))

    def forward(self, feats, coors, adj_mat=None, edges=None, mask=None, return_coor_changes=False):
