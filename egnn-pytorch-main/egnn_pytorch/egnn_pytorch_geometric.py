...
class Attention_Sparse(Attention):
    ...
    def sparse_forward(self, x, context, batch=None, batch_uniques=None, mask=None):
        ...
        if batch_uniques[0].shape[0] == 1:
            ...
            return self.forward(x, context, mask=None).squeeze()
        else:
            x_list = []
            aux_count = 0
            for bi, n_idxs in zip(*batch_uniques):
                x_list.append(self.sparse_forward(x[aux_count:aux_count + n_i], context[aux_count:aux_count + n_idxs], batch_uniques=(bi.unsqueeze(-1), n_idxs.unsqueeze(-1))))
...
class EGNN_Sparse(MessagePassing):
    ...
    def __init__(self, feats_dim, pos_dim=3, edge_attr_dim=0, m_dim=16, fourier_features=0, soft_edge=0, norm_feats=False, norm_coors=False, norm_coors_scale_init=0.01, update_feats=True, update_coors=True, dropout=0.0, coor_weights_clamp_value=None, aggr='add', **kwargs):
        ...
        self.edge_mlp = nn.Sequential(nn.Linear(self.edge_input_dim, self.edge_input_dim * 2), self.dropout, SiLU(), nn.Linear(self.edge_input_dim * 2, m_dim), SiLU())
        ...
        self.node_mlp = nn.Sequential(nn.Linear(feats_dim + m_dim, feats_dim * 2), self.dropout, SiLU(), nn.Linear(feats_dim * 2, feats_dim)) if update_feats else None
        self.coors_mlp = nn.Sequential(nn.Linear(m_dim, m_dim * 4), self.dropout, SiLU(), nn.Linear(self.m_dim * 4, 1)) if update_coors else None
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_attr: OptTensor=None, batch: Adj=None, angle_data: List=None, size: Size=None) -> Tensor:
        """ Inputs: 
        ...
        hidden_out, coors_out = self.propagate(edge_index, x=feats, edge_attr=edge_attr_feats, coors=coors, rel_coors=rel_coors, batch=batch)
...
class EGNN_Sparse_Network(nn.Module):
    """Sample GNN model architecture that uses the EGNN-Sparse
    ...
    def __init__(self, n_layers, feats_dim, pos_dim=3, edge_attr_dim=0, m_dim=16, fourier_features=0, soft_edge=0, embedding_nums=[], embedding_dims=[], edge_embedding_nums=[], edge_embedding_dims=[], update_coors=True, update_feats=True, norm_feats=True, norm_coors=False, norm_coors_scale_init=0.01, dropout=0.0, coor_weights_clamp_value=None, aggr='add', global_linear_attn_every=0, global_linear_attn_heads=8, global_linear_attn_dim_head=64, num_global_tokens=4, recalc=0):
        super().__init__()
        ...
        for i in range(n_layers):
            layer = EGNN_Sparse(feats_dim=feats_dim, pos_dim=pos_dim, edge_attr_dim=edge_attr_dim, m_dim=m_dim, fourier_features=fourier_features, soft_edge=soft_edge, norm_feats=norm_feats, norm_coors=norm_coors, norm_coors_scale_init=norm_coors_scale_init, update_feats=update_feats, update_coors=update_coors, dropout=dropout, coor_weights_clamp_value=coor_weights_clamp_value)
            is_global_layer = self.has_global_attn and i % self.global_linear_attn_every == 0
            if is_global_layer:
                attn_layer = GlobalLinearAttention(dim=self.feats_dim, heads=global_linear_attn_heads, dim_head=global_linear_attn_dim_head)
                self.mpnn_layers.append(nn.ModuleList([layer, attn_layer]))
            else:
                self.mpnn_layers.append(layer)

    def forward(self, x, edge_index, batch, edge_attr, bsize=None, recalc_edge=None, verbose=0):
        ...
        for i, layer in enumerate(self.mpnn_layers):
            ...
            if self.recalc and (i % self.recalc == 0 and (not i == len(self.mpnn_layers) - 1)):
                edge_index, edge_attr, _ = recalc_edge(x)
                edges_need_embedding = True
        return x
